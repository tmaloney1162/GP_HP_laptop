...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exsl="http://exslt.org/common" extension-element-prefixes="exsl"
	...
	<xsl:template match="/">
		<xsl:processing-instruction name="xep-pdf-crop-offset">0.5in 0.5in</xsl:processing-instruction>
	...
	<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
			<fo:layout-master-set>
				<fo:simple-page-master master-name="front-page" page-width="9in" page-height="6in">
					<fo:region-body padding="0in" />
					<fo:region-before extent="6in" padding="0in"/>
					<fo:region-after extent="6in" padding="0in" />					
			...
			<xsl:for-each select="//record">
				<xsl:if test="not(fullname='FULLNAME')">
					<fo:page-sequence master-reference="front-page" force-page-count="no-force">
				      ...
				      <fo:block padding="-0.25in -0.25in" margin-top="0in" margin-left="0in">
	...
	<xsl:template name="front">
		  <fo:block-container height="6in" background-image="url('C:\GP\Grainger\reorder\panelImages\MailPanel6.5x9.5.pdf')" background-repeat="no-repeat" padding-top="0.25in" padding-bottom="0.25in" padding-right="0.25in">
			  <fo:block font="8pt Arial" >
			  	<fo:block margin-left="7.75in" margin-top="0.25in">
			  		<fo:external-graphic src="url('C:\GP\Grainger\reorder\panelImages\PRSRTMKTG.jpg')"/>
			  	</fo:block>
		      <fo:block font="10pt Arial" margin-left="5.25in" margin-top="1.75in">
	...
	<xsl:template name="back">
		  <fo:block-container height="6in" background-image="url('C:\GP\Grainger\reorder\panelImages\BackPanel6.5x9.5.pdf')" background-repeat="no-repeat" padding-top="0.25in" padding-bottom="0.25in" padding-right="0.25in">
